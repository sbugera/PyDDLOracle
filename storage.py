...
def get_full_storage(
    ...
    if str(tablespace_name) != "nan" and local_index != "YES":
        ...
        storage = statement.replace(
            "<:1>", get_case_formatted(tablespace_name, "identifier")
        )
    if str(pct_free) != "nan":
        statement = get_case_formatted(
    ...
    if initial_extent and str(initial_extent) not in ("nan", "DEFAULT", "-1"):
        ...
        storage_tmp += statement.replace(
            "<:1>", get_size_formatted(initial_extent)
        )
    if next_extent and str(next_extent) not in ("nan", "DEFAULT", "-1"):
        ...
        storage_tmp += statement.replace(
            "<:1>", str(int(next_extent / 1024 / 1024)) + "M"
        )
    ...
    if buffer_pool and buffer_pool != "DEFAULT2":
        ...
        storage_tmp += statement.replace(
            "<:1>", get_case_formatted(buffer_pool, "identifier")
        )
    if flash_cache and flash_cache != "DEFAULT":
        ...
        storage_tmp += statement.replace(
            "<:1>", get_case_formatted(flash_cache, "identifier")
        )
    if cell_flash_cache and cell_flash_cache != "DEFAULT":
        ...
        storage_tmp += statement.replace(
            "<:1>", get_case_formatted(cell_flash_cache, "identifier")
        )
