...
class Table:
    ...
    def __init__(
        ...
        self.def_tablespace_name = (
            part_table.def_tablespace_name if part_table else ""
        )
        ...
        self.def_compress_for = (
            part_table.def_compress_for if part_table else ""
        )
        ...
        self.def_pct_increase = (
            part_table.def_pct_increase if part_table else ""
        )
        ...
        self.def_cell_flash_cache = (
            part_table.def_cell_flash_cache if part_table else ""
        )
    ...
    def get_collation(self):
        ...
        if (
            ...
            and self.default_collation != "USING_NLS_COMP"
    ...
    def get_storage(self):
        ...
        if c.conf["storage"]["storage"] == "no_storage":
            ...
            storage = f"\n{get_case_formatted('TABLESPACE', 'keyword')} {get_case_formatted(self.tablespace_name, 'identifier')}"
        elif (
            c.conf["storage"]["storage"] == "only_tablespace"
            and self.partitioned == "YES"
        ):
            storage = f"\n{get_case_formatted('TABLESPACE', 'keyword')} {get_case_formatted(self.def_tablespace_name, 'identifier')}"
        elif (
            ...
            storage = get_full_storage(
                ...
                self.def_ini_trans,
                self.def_max_trans,
                self.def_min_extents,
                self.def_max_extents,
                self.def_pct_increase,
    ...
    def get_partitioning(self):
        ...
        partitioning = Partitioning(
            self.part_table, self.part_key_columns, self.tab_partitions
        )
    ...
    def get_constraints(self):
        ...
        if c.conf["constraints"] == "yes":
            ...
            for i, constraint_row in enumerate(
                self.tab_constraints.itertuples()
            ):
                constraint_columns = self.tab_constraint_columns[
                    self.tab_constraint_columns["constraint_name"]
        ...
        if c.conf["comments"]["comments"] == "yes":
            for comment_row in self.comments.itertuples():
                if (
                    ...
                    or str(comment_row.column_name) == "nan"
                ):
                    statement = get_case_formatted(
                        f"COMMENT ON TABLE <:1> IS '<:2>';\n{end_line_char}",
                        "keyword",
                    )
                    comments += statement.replace(
                        "<:1>", self.table_full_name
                    ).replace("<:2>", comment_row.comments)
                else:
                    statement = get_case_formatted(
                    ...
                    comments += (
                        statement.replace("<:1>", self.table_full_name)
    ...
    def get_grants(self):
        ...
        if c.conf["grants"] == "yes":
            ...
            for tab_grants_group in tab_grants_grouped.itertuples():
                grantee = get_case_formatted(
                    tab_grants_group.grantee, "identifier"
                )
                privileges = get_case_formatted(
                    tab_grants_group.privilege, "keyword"
                )
                grant_option = ""
                ...
                grant = get_case_formatted(
                    f"GRANT <:1> ON <:2> TO <:3>{grant_option};\n\n", "keyword"
                )
                grants += prompt + (
                    grant.replace("<:1>", privileges)
    ...
    def generate_ddl(self):
        ...
        ddl = get_prompt("Table ", self.table_full_name) + statement.replace(
            "<:1>", self.table_full_name
        )
